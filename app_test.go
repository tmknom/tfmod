...
func TestApp_Run_Dependencies(t *testing.T) {
	...
	cases := []struct {
		...
		{
			input:    []string{"dependencies", "--state-dirs", "env/prd", "--base=testdata/terraform", "--format=json"},
...
func TestApp_Run_Dependents(t *testing.T) {
	...
	cases := []struct {
		...
		{
			input:    []string{"dependents", "--module-dirs", "module/foo", "--base=testdata/terraform", "--format=text"},
